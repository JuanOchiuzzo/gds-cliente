...
import { Search, Plus, Phone, MessageCircle, SlidersHorizontal } from 'lucide-react';
...
export default function LeadsPage() {
  ...
  const handleCreate = async () => {
    ...
    setShowNew(false);
    setNewName(''); setNewPhone(''); setNewEmail(''); setNewValue('');
  };
    ...
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-4">
      ...
      {showFilters && (
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {['all', 'novo', 'qualificado', 'visita_agendada', 'proposta', 'negociacao', 'fechado', 'perdido'].map((s) => (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--sf-text-tertiary)]">{selected.email}</p>
                <p className="text-sm text-[var(--sf-text-tertiary)]">{selected.phone}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2.5">
              {[
                ...
                { label: 'Etapa', value: getLeadStageLabel(selected.stage) },
              ].map((i) => (
